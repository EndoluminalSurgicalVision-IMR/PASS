...
import models.moment_tta.diffmoments as diffmoments
import argparse
...
def get_mom_posmed(nclass, mom, sizes, th):
    med = [np.round(
        np.median(
            [mom[i][j].cpu().numpy() for i in range(0, len(mom)) if sizes[i][j] > th[0][j] and sizes[i][j] < th[1][j]]),
        2) for
        j in range(0, nclass)]
    return med
...
def adjust_learning_rate_mm(optimizer, i_iter, learning_rate, num_steps, power):
