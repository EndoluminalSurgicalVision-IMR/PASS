...
def ae_collate_fn_tr(batch):
    ...
    data_dict = {'data': image, 'seg': label, 'name': name}
    tr_transforms = get_AE_transform()
    data_dict = tr_transforms(**data_dict)
    data_dict['org_data'] = image_org
    return data_dict


def ae_collate_fn_tr_prostate(batch):
...
def target_collate_fn_tr_fda(batch):
    image, _, name = zip(*batch)
    image = np.stack(image, 0)
    # label = np.stack(label, 0)
    name = np.stack(name, 0)
...
def prostate_target_collate_fn_tr_fda(batch):
...
def target_collate_fn_tr(batch):
    image, _, name = zip(*batch)
    image = np.stack(image, 0)
    # label = np.stack(label, 0)
    name = np.stack(name, 0)
